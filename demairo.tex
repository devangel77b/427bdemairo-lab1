...
Results were initially visualized in Python using the \texttt{numpy}, \texttt{scipy}, and \texttt{matplotlib} libraries \cite{numpy,scipy,matplotlib}. Subsequent statistical analysis \cite{starnes:2015:practice} was perfomed in R \cite{R} using the \texttt{ggplot2} and \texttt{dplyr} libraries \cite{ggplot2,dplyr}. All data and analysis code is available at \url{https://github.com/devangel77b/427bdemairo-lab1}. 
...
\begin{figure}
\begin{center}
\includesvg{fig21.svg}
\end{center}
\caption{Distance traveled down the inclined plane versus time for five trials for the baseball and the ping pong ball. Differences between baseball and ping pong ball are statistically significant (nested ANOVA, $p=\num{7.19e-11}$).}
\label{fig:21}
\end{figure}
\begin{figure}
\begin{center}
\includesvg{fig22.svg}
\end{center}
\caption{Distance traveled down the inclined plane versus time squared for five trials fo the baseball and the ping pong ball. Differences between baseball and ping pong ball are statistically significant (nested ANOVA, $p=\num{7.19e-11}$).}
\label{fig:22}
\end{figure}
...
The mean times across five trials were used to compute representative acceleration values. For the baseball, the mean time to travel \qty{90}{in} was \qty{2.221}{\second}, corresponding to an average acceleration of approximately \qty{36.49}{in\per\second\squared}. For the ping pong ball, the mean time was \qty{2.332}{\second}, corresponding to an average acceleration of approximately \qty{33.10}{in\per\second\squared}. 

The measured accelerations, from linear regression of \cref{fig:22}, were approximately \qty{35.2\pm0.3}{in\per\second\squared} for baseball and \qty{32.0\pm0.3}{in\per\second\squared} for ping pong. %Differences between baseball and ping pong ball were significant (nested ANOVA, $p=\num{7.19e-11}$).
 While the measured accelerations were not identical, both objects demonstrated motion consistent with constant acceleration, as evidenced by the quadratic displacement-time relationship. The observed differences in acceleration magnitude are consistent with expectations for different moments of inertia during rolling motion and do not detract from the validity of the constant-acceleration model.
